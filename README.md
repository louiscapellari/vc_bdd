...
classDiagram
  ...
  class NoeudViaire { +id_noeud +geom }
  class CoursEau { +id_cours_eau +nom }
  class TronconHydro { +id_th +geom +regime }
  class NoeudHydro { +id_nh +geom }
  class SurfHydro { +id_sh +geom +type }
  class BassinVersant { +id_bv +geom }
  class Zonage {
    +id_zonage
    +geom
    +theme
  ...
  class MilieuNaturel { +id_milieu +geom +type +source }
  class Sentier { +id_sentier +geom +typologie +source }
  class POITourisme { +id_poi +geom +categorie +nom +source }
  class MNT { +id_mnt +grid +resolution }
  class MNS { +id_mns +grid +resolution }
  class CourbeNiveau { +id_cn +geom +altitude }

  Parcelle "1" -- "0..*" Batiment : porte/implantation
  Parcelle "1" -- "0..*" Adresse : peut_porter
  Voie "1" -- "0..*" Adresse : nomme
  Voie "1" -- "0..*" TronconRoute : reference
  TronconRoute "0..*" -- "2..*" NoeudViaire : connecte

  CoursEau "1" -- "0..*" TronconHydro : compose
  TronconHydro "0..*" -- "2..*" NoeudHydro : connecte
  BassinVersant "1" -- "0..*" SurfHydro : contient

  Zonage "0..*" -- "0..*" Parcelle : couvre
  Zonage "0..*" -- "0..*" Batiment : contraint
  Zonage "0..*" -- "0..*" TronconRoute : reglemente
  Zonage "0..*" -- "0..*" MilieuNaturel : protege

  Sentier "0..*" -- "0..*" POITourisme : dessert
  Sentier "0..*" -- "0..*" TronconRoute : croise

  MNT "1" -- "0..*" CourbeNiveau : isocote
  MNT "1" -- "0..1" MNS : derive
